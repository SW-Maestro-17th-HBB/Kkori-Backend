...
- 각 도메인의 하위 패키지: `controller`(@RestController) / `service` / `repositoryservice`(영속성 접근 계층 — 다음 항목 참조, JPA repository 외의 네이티브 접근 클래스도 여기) / `repository`(JPA) / `api`(**Swagger 문서화 인터페이스** — `@Tag`/`@Operation` 담긴 interface, controller가 `implements`) / `dto`(요청·응답) / `domain`(@Entity, `global.entity.BaseEntity` 상속) / `scheduler`(`@Scheduled` 트리거 — service의 실행 메서드를 호출만 하고 로직을 두지 않음, `app.batch.enabled` 조건 — 테스트는 이 스위치로 빈 등록을 막고 실행 메서드를 직접 호출) / `purge`(탈퇴 파기 — 오케스트레이터·`PurgeStep` 계약은 `user/purge`, 각 도메인은 자기 데이터의 파기 단계 구현체를 `<도메인>/purge`에 둔다: 의존 방향 `<도메인>.purge → user.purge`, user는 타 도메인을 컴파일 시점에 의존하지 않음)
- **service와 repository 사이에 repositoryService 계층을 둔다** — DB에서 가져오고 저장하는 영속성 접근 작업(조회·저장, 존재 검증과 도메인 예외 변환 포함)은 repositoryService(패키지명은 Java 관례대로 소문자 `repositoryservice`, 클래스명은 `<도메인>RepositoryService`, 예: `ReportRepositoryService` — 한 테이블의 접근이 커지면 그 테이블 전용 클래스로 분리할 수 있다, 예: user 도메인의 `DeletionLogRepositoryService`)가 담당하고, service는 repositoryService만 의존해 비즈니스 흐름을 조립한다. 의존 방향: `service → repositoryService → repository` (service가 repository를 직접 의존하지 않음). 전 도메인 적용 완료(HBB1-325) — main 소스에서 raw repository 의존은 `repositoryservice` 패키지 안에만 존재한다(테스트의 데이터 준비·단언용 @Autowired는 예외)
