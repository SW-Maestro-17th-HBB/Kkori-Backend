...
- **타 도메인 영속성 접근도 해당 도메인의 repositoryService를 경유한다** — 타 도메인의 raw repository import 금지 (예: 세션의 user 행 잠금은 `UserRepositoryService.findWithLockById`). repositoryService는 자기 도메인의 repository만 의존하고 타 repositoryService를 의존하지 않는다(도메인 간 빈 순환 차단). repositoryService는 트랜잭션을 소유하지 않는다 — 잠금 메서드는 호출자의 트랜잭션 안에서만 호출(비관적 잠금의 수명이 트랜잭션에 묶임)
