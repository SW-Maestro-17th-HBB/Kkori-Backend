...
리포트 도메인은 면접 세션이 끝나면 질문-답변 기록을 AI로 평가해 종합 리포트(총평·영역별 점수·답변별 피드백·약점 태그·개선 과제)를 비동기로 생성하고, 사용자가 목록·상세·질문-답변 타임라인·통계로 과거 면접을 복기하는 기능 영역이다. **생성 수명주기 전체 — 리포트(PENDING) 로우·Job·스냅샷 생성부터 평가·저장·COMPLETED 전이까지 — 는 Python AI Worker가 면접 도메인(에이전트)이 발행하는 리포트 생성 요청(Redis Stream)을 직접 소비해 수행한다.** Spring(리포트 도메인)은 조회 API·접근 권한·재생성 API를 담당하고, 상태 이벤트를 소비해 SSE로 사용자에게 실시간 전달한다.

핵심 데이터 흐름: `세션 정상 종료 → 대본(transcripts) 저장 → 리포트 생성 요청 발행(면접 도메인 에이전트 — 대본 저장 성공 후) → Worker가 직접 소비: 리포트(PENDING)·Job·스냅샷 생성 → 1단계: transcripts·이력서 근거로 텍스트 평가·저장 → (음성 분석 요청 소비) 2단계: 녹음 파일(S3 호환)로 전달력 산출 → 리포트 COMPLETED → 상태 이벤트 → Spring이 SSE push`
...
| 상태 | 의미 |
| --- | --- |
| PENDING | 생성 요청 소비로 리포트가 생성되어 평가 대기 중 |
...
면접 도메인(에이전트)은 면접 세션이 **정상 종료**될 때 대본(`INTERVIEW_TRANSCRIPTS`) 저장을 마친 뒤 Redis Stream에 **리포트 생성 요청**을 발행하고, **Python Worker가 이 스트림을 직접 소비해** 리포트(PENDING)·Job 레코드를 생성하고 스냅샷 컬럼을 채운 뒤 곧바로 텍스트 분석을 시작한다 — 중간에 Spring을 거치는 전달 단계가 없다. 사용자가 호출하는 생성 API는 없다 — 생성은 생성 요청이 트리거하고, 사용자 개입은 FAILED 재생성뿐이다. 트리거를 "세션이 끝났다"는 사실 통지가 아니라 "리포트가 필요하다"는 요청으로 명명한 것은 이력서의 `resume.parse.requested`와 같은 요청형 계약이기 때문이다(발행 조건이 곧 리포트 정책을 담는다 — 2026-07-28 확정).

- **트리거 범위**: 생성 요청은 **실전(30분) 면접의 정상 종료 세션**에 대해서만 발행된다 — **빠른(5분) 면접은 리포트를 발행하지 않는다(2026-07-24 확정)**. 발행 조건은 발행자인 면접 도메인 소관(중도 이탈 세션의 종료 확정 정책은 면접 도메인 확정 후 재검토). Worker는 소비 시점에 세션 대본(`INTERVIEW_TRANSCRIPTS` — 세션당 1행, 발화 배열 JSON)을 확인해 **사용자 발화가 0건이면 리포트를 생성하지 않고 ACK만 한다**(평가할 대상이 없음). **대본 행 자체가 없으면 리포트 로우를 만들기 전에 로그를 남기고 스킵 ACK한다(2026-07-30 변경)** — 발행이 대본 저장 성공 뒤에만 일어난다는 순서가 발행 측 구현으로 보장되므로(아래 발행 측 요구사항), 대본 없는 요청은 "아직 저장 전"이 아니라 실제 데이터가 없는 비정상 메시지다. 재전달을 기다리게 두면 같은 메시지가 무의미하게 반복 전달된다.
- **시작 신호는 요청 2개다 (2026-07-23 확정)**: 면접 도메인은 ①세션이 정상 종료될 때 **리포트 생성 요청**을, ②녹음 파일이 S3에 올라간 뒤 **음성 분석 요청**을 각각 발행하며, **둘 다 Worker가 직접 소비한다.** 텍스트 분석은 ①로 바로 시작하므로 리포트 생성이 녹음 업로드를 기다리지 않고, 음성 분석은 ②가 와야 시작한다.
- **세션:리포트 = 1:1, 소비 멱등**: 같은 세션에 리포트는 1개만 존재한다. 생성 요청 스트림은 Consumer Group 기반 **at-least-once**라 동일 요청이 중복 전달될 수 있으므로 Worker의 소비 처리는 **sessionId 기준 멱등**이어야 한다 — `interview_session_id` 유니크 제약으로 중복 생성을 방어하고, 이미 완결(COMPLETED/FAILED)된 리포트가 있으면 아무것도 하지 않고 ACK한다.
- **소유자·이력서 정보는 메시지가 아니라 DB에서 읽는다 (2026-07-30 확정)**: 생성 요청의 필드는 `sessionId` 하나뿐이고, Worker는 세션 행(`interview_session`)에서 소유자(user_id)와 사용 이력서(resume_id)를 조회해 리포트 로우를 만든다 — 메시지는 포인터이며 진실 원천은 DB 하나다.
- **스냅샷 컬럼**: Worker가 리포트 로우 생성(PENDING) 시점에 `resume_file_name_snapshot`(이력서 원본 파일명)을 복사한다. 이후 원본이 변경·삭제되어도 목록 표시는 스냅샷으로 성립한다. **면접 유형·시간 필드는 두지 않는다 (2026-07-24 확정)** — 리포트는 실전 30분 면접에만 발행되므로 유형과 시간이 모든 리포트에서 동일한 상수라 저장·전달할 정보가 없다. "실전 30분" 라벨은 프론트가 상수로 표시한다. 빠른 면접에도 리포트를 발행하게 되면 그때 유형 구분 필드를 추가한다.
- **Worker 파이프라인 (2단계)**:
  - **1단계 텍스트 분석** (생성 요청으로 시작): 대본 확인 → **리포트(PENDING)·Job·스냅샷 생성** → transcripts·이력서 데이터 로드 → 답변별 평가(텍스트 3축 점수 + 피드백 + 약점 태그 + **개선 과제** + 이력서 근거) → 영역 점수 산정 → 총평 생성 → 약점 태그 요약 집계 → 산출물을 한 트랜잭션으로 저장하고 `text_analyzed_at`을 기록. 재시도하면 1단계를 처음부터 다시 수행한다(저장 전 기존 산출물이 있으면 지우고 다시 저장).
  - **2단계 음성 분석** (음성 분석 요청으로 시작): 녹음 파일 다운로드 → 사용자 음성의 말 속도·멈춤 계산 → `REPORTS.delivery_score` 갱신 + `audio_analyzed_at` 기록.
  ...
  - 음성 분석 요청 처리 시 리포트 로우가 없으면 대본을 확인한다: **사용자 발화가 0건이면 "리포트 없음"이 정상이므로 그대로 ACK**하고(무응답 세션의 음성 요청이 영구 재전달되는 것 방지), 발화가 있는데 로우가 없으면(소비 지연) ACK하지 않고 재전달을 기다린다.
  - **이미 COMPLETED된 리포트에 뒤늦게 도착한 음성 분석 요청은 무시하고 ACK만 한다** (음성 없이 완성 처리된 경우 포함 — 전달력은 빈 값 유지). 사용자에게 이미 보여준 점수는 사후에 바뀌지 않는다. 이를 위해 2단계의 delivery 갱신·완성 전환 UPDATE는 **COMPLETED가 아닌 리포트에만** 적용되도록 조건을 건다.
...
- **실패·재시도**: Worker가 소비하는 스트림(생성 요청·음성 분석 요청·재생성 요청)은 모두 Consumer Group 기반 at-least-once이며, 처리는 sessionId(또는 reportId) 기준 멱등이어야 한다. **모든 생성 요청은 반드시 COMPLETED/FAILED로 끝나거나 스킵 ACK된다** — Worker는 ACK 없이 방치된 메시지를 회수(XAUTOCLAIM)해 재평가하고, 재전달 횟수가 임계를 넘으면 FAILED로 종결한다(`failed_reason` 기록). idle time·임계값·내부 재시도 수치는 Worker PRD 소관. 이력서와 달리 트리거가 시스템이므로 **자동 재시도를 허용**하되, FAILED 종결 후의 복구 주체는 사용자다(아래 재생성).
...
- Worker가 생성 요청을 소비하면 리포트(PENDING)·Job 레코드가 생성되고 텍스트 분석이 시작되는지 확인
- 대본에 사용자 발화가 0건인 세션의 생성 요청은 리포트 생성 없이 ACK만 되는지 확인
- 대본 행이 없는 생성 요청은 리포트 로우 생성 없이 로그 후 스킵 ACK되는지 확인
- 같은 세션의 생성 요청이 중복 전달되어도 리포트가 1개만 존재하는지 확인 (`interview_session_id` 유니크)
- 소비 처리 도중 실패 시 ACK되지 않고 재전달로 복구되는지 확인
- 정상 종료 후 유예 시간이 지나도 리포트 로우가 없는(대본은 있는) 세션을 조정 배치가 감지해 생성 요청 소비와 동일한 처리(생성·분석)를 수행하는지 확인
- 조정 배치가 PENDING/PROCESSING/FAILED 리포트가 존재하는 세션을 건드리지 않는지 확인 (로우 부재 기준)
- 조정 배치와 지연된 이벤트 소비가 겹쳐도 리포트가 1개만 생성되는지 확인
- 리포트 생성 시점에 스냅샷 컬럼(이력서 파일명)이 채워지는지 확인
...
- 음성 분석 요청이 리포트 로우 생성보다 먼저 도착하면(발화 있는 세션) ACK되지 않고 재전달되는지 확인
- 사용자 발화 0건 세션의 음성 분석 요청은 리포트 없이 ACK되는지 확인
...
- 재생성 시 이전 런의 텍스트 경로 결과 컬럼(실패 사유·overall·총평·태그 요약·완료 시각들)이 초기화되는지 확인
...
- 조정 배치의 실행 주기·유예 시간: **미정** — 값은 추측으로 정하지 않고 실측 후 확정한다(생성 누락 감지의 유예는 소비 지연 실측, 음성 유예 완성의 유예는 녹음 업로드 소요 실측 기준)
...
- **"실전 면접만 발행" 조건의 근거인 면접 유형 정보는 세션 도메인 소관** — 면접 유형 선택(HBB1-18)을 설계하는 면접 도메인이 세션에 기록하고 발행 조건 판정에 사용한다. 리포트 도메인은 유형·시간을 저장·전달하지 않는다.
- **녹음·음성 분석의 남은 확정 사항** — 자체 호스팅 LiveKit의 Egress 설정(사용자 트랙 분리 녹음), 음성 분석 요청의 발행 시점·필드(면접 도메인 소관), 음성 유예 시간 값, audio_usage 동의와 녹음의 연결. 확정 전까지 Worker는 텍스트 3축으로만 동작하고 delivery_score는 null이다. **전달력 점수를 항상 제공하는 방법**(대본 발화 타임스탬프 기반 근사 등)은 MVP에서 도입하지 않고 추후 논의한다(2026-07-27 확정 — 음성 실패·유예 초과 시 delivery null 완성을 유지).
...
- 목록 항목은 **REPORTS 단독으로 구성한다**(조인 없음) — overall_score, 상태, 스냅샷 컬럼(이력서 파일명), weakness_tag_summary, 생성·완료 시각. 스냅샷 비정규화가 이 요구를 위한 설계다.
...
- 필터: `status`(생성 상태). 생성 중(PENDING/PROCESSING)·FAILED 리포트도 목록에 노출된다(상태 표시는 프론트 소관).
- 목록 화면의 KPI·추이·약점 분포는 목록 응답이 아니라 통계 API(§6)가 담당한다.
- 프론트가 표시하는 "제목"(예: "백엔드 개발자 · 기술 면접")과 날짜·시간 포맷은 서버 필드(이력서 파일명·시각)로 프론트가 구성한다 — 서버는 title 필드를 제공하지 않는다. **"실전 30분" 라벨은 프론트가 상수로 표시한다(모든 리포트가 실전 면접이므로 서버는 유형·시간 필드를 제공하지 않는다).** 모든 시각 필드는 ISO-8601이다.
...
- status 필터 지정 시 해당 상태의 리포트만 반환되고, 잘못된 값은 400(INVALID_INPUT_VALUE)인지 확인
