...
리포트 도메인은 면접 세션이 끝나면 질문-답변 기록을 AI로 평가해 종합 리포트(총평·영역별 점수·답변별 피드백·약점 태그·개선 과제)를 비동기로 생성하고, 사용자가 목록·상세·질문-답변 타임라인·통계로 과거 면접을 복기하는 기능 영역이다. 리포트 도메인(Spring)은 **세션 도메인이 발행하는 세션 종료 이벤트(Redis Stream)의 컨슈머**로서 리포트·Job 레코드를 생성하고 Worker를 향한 생성 요청을 발행하며, 상태 관리·조회 API·접근 권한을 담당한다. 실제 평가(답변별 4개 축 평가 → 점수 산정 → 총평·개선 과제 생성)는 이력서 분석과 동일하게 Redis Stream으로 분리된 Python AI Worker가 수행한다. 생성 진행 상태는 상태 이벤트를 Spring이 소비하여 SSE로 사용자에게 실시간 전달한다.

핵심 데이터 흐름: `세션 정상 종료 → 세션 종료 이벤트 발행(세션 도메인, Redis Stream) → 리포트 도메인이 소비: 리포트(PENDING)·Job 생성 + 생성 요청 발행 → Worker 1단계: transcripts·이력서 근거로 텍스트 평가·저장 → Worker 2단계: 녹음 파일(S3 호환)로 전달력 산출 → 리포트 COMPLETED → 상태 이벤트 → Spring이 SSE push`
...
- 축 점수는 jsonb가 아닌 **개별 컬럼**으로 저장한다(REPORT_SCORES·REPORT_FEEDBACKS 동일) — 통계 집계(§6 축별 평균)가 SQL AVG로 가능해야 하고, 0~100 CHECK·NULL 제약을 컬럼 단위로 선언하기 위함.
...
- **오디오는 best-effort (완성 보증 폴백)**: 오디오가 **유예 시간 내 준비되지 않으면 텍스트 3축만으로 COMPLETED 처리한다(delivery_score null)** — 녹음 사고가 리포트를 영구 미완성에 머물게 하거나 복기 기록을 잃게 하지 않는다. 총평·약점 태그·개선 과제는 텍스트 단계 소관으로 한정하므로, 음성 단계가 늦거나 생략되어도 달라지는 것은 delivery_score(와 그에 따른 overall) 하나뿐이다.
- **녹음 파일은 전 환경 S3 호환 저장소를 경유한다 (2026-07-23 확정)**: 자체 호스팅 LiveKit의 Egress(사용자 트랙 분리 녹음)가 로컬은 MinIO, prod는 실제 S3의 전용 prefix(예: `recordings/`)에 업로드하고, Worker는 전 환경 동일하게 S3 클라이언트로 다운로드한다(이력서 PDF와 같은 패턴 — 코드 경로 단일). 로컬 파일·볼륨 공유 방식은 호스트 결합·유실·파기 통제 문제로 채택하지 않는다.
...
- 이력서와 동일하게 Worker가 처리 단계 진입 시 상태를 직접 갱신하고 상태 이벤트를 발행한다. PENDING 기록(세션 종료 이벤트 소비 시)과 생성 요청 발행만 Spring이 수행한다.
...
세션 도메인은 면접 세션이 **정상 종료**(`ended_at` 확정)될 때 Redis Stream에 세션 종료 이벤트를 발행하고, 리포트 도메인은 이 스트림의 컨슈머로서 이벤트를 소비해 리포트(PENDING)·Job 레코드를 생성하고 스냅샷 컬럼을 채운 뒤 Worker를 향한 생성 요청을 발행한다. 사용자가 호출하는 생성 API는 없다 — 생성은 세션 종료 이벤트가 트리거하고, 사용자 개입은 FAILED 재생성뿐이다.

- **트리거 범위**: 세션 종료 이벤트는 정상 종료 세션에 대해서만 발행된다(발행 조건은 발행자인 세션 도메인 소관 — 중도 이탈 세션의 종료 확정 정책은 면접 도메인 확정 후 재검토). 리포트 도메인은 소비 시점에 세션 대본(`INTERVIEW_TRANSCRIPTS` — 세션당 1행, 발화 배열 JSON)을 확인해 **사용자 발화가 0건이면 리포트를 생성하지 않고 ACK만 한다**(평가할 대상이 없음). **대본 행 자체가 없으면 스킵이 아니라 처리 실패로 취급**해 ACK하지 않는다 — "아직 저장 전"과 "답변 없음"을 구별하기 위함이며, 대본 저장이 세션 종료 이벤트 발행보다 선행된다는 순서 보장은 면접 도메인에 요구사항으로 전달한다(**미정**).
- **시작 신호는 이벤트 2개다 (2026-07-23 확정)**: 세션 도메인은 ①세션이 정상 종료될 때 **세션 종료 이벤트**를, ②녹음 파일이 S3에 올라간 뒤 **음성 업로드 이벤트**를 각각 발행한다. 텍스트 분석은 ①로 바로 시작하므로 리포트 생성이 녹음 업로드를 기다리지 않고, 음성 분석은 ②가 와야 시작한다.
- **세션:리포트 = 1:1, 소비 멱등**: 같은 세션에 리포트는 1개만 존재한다. 세션 종료 이벤트 스트림은 Consumer Group 기반 **at-least-once**라 동일 이벤트가 중복 전달될 수 있으므로 소비 처리는 **sessionId 기준 멱등**이어야 한다 — `interview_session_id` 유니크 제약으로 중복 생성을 방어하고, 이미 리포트가 있으면 아무것도 하지 않고 ACK한다.
- **스냅샷 컬럼**: 리포트 생성(PENDING) 시점에 목록 조회용 값을 복사한다 — `resume_file_name_snapshot`(이력서 원본 파일명), `interview_type_snapshot`(면접 유형), `interview_duration_minutes`(`ended_at - started_at`, 분). 이후 원본이 변경·삭제되어도 목록 표시는 스냅샷으로 성립한다.
- **Worker 파이프라인 (2단계)**:
  - **1단계 텍스트 분석** (세션 종료 이벤트로 시작): transcripts·이력서 데이터 로드 → 답변별 평가(텍스트 3축 점수 + 피드백 + 약점 태그 + **개선 과제** + 이력서 근거) → 영역 점수 산정 → 총평 생성 → 약점 태그 요약 집계 → 산출물을 한 트랜잭션으로 저장하고 `text_analyzed_at`을 기록. 재시도하면 1단계를 처음부터 다시 수행한다(저장 전 기존 산출물이 있으면 지우고 다시 저장).
  ...
  - 음성 업로드 이벤트가 리포트 로우 생성보다 먼저 도착하면 ACK하지 않고 재전달을 기다린다(대본 행 부재 규칙과 동일).
...
- **소비 측 신뢰성**: 리포트 도메인은 세션 종료 이벤트 소비 → 리포트·Job 생성 → 생성 요청 발행을 하나의 처리 단위로 수행하고, **전부 성공했을 때만 ACK한다**. 도중 실패(DB 오류·발행 실패)하면 ACK하지 않아 스트림 재전달로 복구되고, 위의 sessionId 멱등이 중복 재처리를 무해화한다 — 컨슈머 쪽은 Outbox가 필요 없다.
- **발행 유실 방어 (Outbox 도입 전까지 2겹)**: 발행 측(세션 종료 커밋 + 발행의 이중 쓰기)은 커밋 후 발행만 실패하면 리포트가 조용히 누락되는 구멍이 있다. 이력서 업로드와 달리 웹훅 처리라 실패를 보고 재시도할 사용자가 없으므로, 다음 2겹으로 방어한다:
  - **웹훅 재전송 (세션 도메인 소관)**: 종료 기록과 발행을 한 처리 단위로 묶고 실패 시 5xx 응답으로 LiveKit 웹훅 재전송을 유도한다. `WEBHOOK_EVENTS` 멱등 기록은 처리 성공 시에만 `processed_at`을 남겨 재전송이 안전하게 재처리되게 한다. — 면접 도메인 설계에 요구사항으로 전달.
  - **조정 배치 (리포트 도메인 소유)**: "정상 종료(`ended_at` 존재) + **리포트 로우 부재** + 유예 시간 경과" 세션을 주기적으로 찾아 리포트를 직접 생성한다(생성 요청 발행 포함 — 소비 처리와 동일 로직 재사용). **판정 기준은 로우 부재이며, 상태(COMPLETED 여부) 기준을 쓰지 않는다** — 상태 기준이면 오래 걸리는 생성·FAILED를 재생성하는 오탐이 생긴다. 컨슈머는 소비 첫 동작으로 PENDING 로우를 만들므로 생성 소요 시간은 판정에 영향이 없고, 배치가 잡는 실패는 "발행 유실" 하나뿐이다. 지연 소비와의 레이스는 `interview_session_id` 유니크가 무해화한다. **PENDING/PROCESSING 정체는 Worker의 메시지 회수(XAUTOCLAIM) 소관, FAILED 복구는 사용자의 재생성 API 소관** — 배치는 이들을 건드리지 않는다. 단 하나의 예외로, **텍스트 분석은 끝났는데 음성 분석이 유예 시간을 넘긴 리포트를 delivery null로 COMPLETED 처리하는 폴백**은 이 배치가 함께 수행한다.
- **Outbox는 추후 도입 예정**: 정식 Outbox는 이력서 도메인 주도로 추후 도입하며, 도입 시 "트랜잭션 내 발행" 지점(이력서 업로드·세션 종료)을 일괄 전환한다. Outbox 릴레이는 본질적으로 at-least-once 재발행이므로 본 문서의 sessionId 소비 멱등이 도입의 전제 조건이며, **도입 후에도 리포트 도메인의 소비 계약(스트림·멱등)은 변하지 않는다.** 조정 배치는 도입 후에도 버그·운영 사고까지 잡는 최후 안전망으로 유지한다.
- **실패·재시도**: 생성 요청 스트림은 Consumer Group 기반 at-least-once이며 Worker 처리는 **reportId 기준 멱등**이어야 한다. 모든 생성 요청은 반드시 COMPLETED 또는 FAILED로 끝난다 — Worker는 ACK 없이 방치된 메시지를 회수(XAUTOCLAIM)해 재평가하고, 재전달 횟수가 임계를 넘으면 FAILED로 종결한다(`failed_reason` 기록). idle time·임계값·내부 재시도 수치는 Worker PRD 소관. 이력서와 달리 트리거가 시스템이므로 **자동 재시도를 허용**하되, FAILED 종결 후의 복구 주체는 사용자다(아래 재생성).
- **FAILED 재생성**(`POST /api/v1/reports/{reportId}/retry`): FAILED 리포트에 한해 기존 산출물을 무효로 하고 상태를 PENDING으로 되돌린 뒤 생성 요청을 다시 발행한다. Job의 `requested_at`을 갱신한다(`retry_count`는 Worker 소관 — 서버는 초기화하지 않는다).

### 실행 조건

- 리포트 도메인의 세션 종료 이벤트 컨슈머가 동작 중이어야 생성이 시작된다(컨슈머 정지 중의 이벤트는 스트림에 남아 재기동 후 처리된다).
- 재생성 API에서 발행 실패 시 500(REPORT_GENERATION_REQUEST_FAILED)으로 응답한다.
- Worker가 transcripts·이력서 데이터를 DB에서 읽을 수 있어야 한다.

### 검증 기준

- 세션 종료 이벤트 소비 시 리포트(PENDING)·Job 레코드가 생성되고 Worker를 향한 생성 요청이 발행되는지 확인
...
- 정상 종료 후 유예 시간이 지나도 리포트 로우가 없는 세션을 조정 배치가 감지해 리포트를 생성하고 생성 요청을 발행하는지 확인
...
- 음성 업로드 이벤트가 리포트 로우 생성보다 먼저 도착하면 ACK되지 않고 재전달되는지 확인
...
- FAILED 리포트에 재생성 요청 시 상태가 PENDING으로 되돌아가고 생성 요청이 재발행되는지 확인
...
- Redis Stream 3종. **스트림별 메시지 스키마의 정의 원천은 계약 record**:
  - **세션 종료 이벤트 스트림(소비)** — 발행자는 세션 도메인. `SessionEndedMessage`(sessionId, userId — 필드 확정은 면접 도메인과 합의, **미정**)
  ...
  - **생성 요청 스트림(발행)** — `ReportGenerationRequestedMessage`(reportId, sessionId, userId — Worker가 DB에서 입력을 직접 읽으므로 포인터만)
  - **상태 이벤트 스트림(소비)** — `ReportStatusChangedMessage`(reportId, userId, status, message — userId는 SSE 라우팅 근거로 Worker가 에코)
  - Python Worker는 음성 업로드·생성 요청·상태 이벤트 record 3개를 계약 문서로 참조
...
- **녹음·음성 분석의 남은 확정 사항** — 자체 호스팅 LiveKit의 Egress 설정(사용자 트랙 분리 녹음), 음성 업로드 이벤트의 발행 시점·필드(세션 도메인 소관), 음성 유예 시간 값, 녹음 파일 보관·파기 정책(권고: 분석 후 즉시 삭제 + S3 수명주기 규칙을 안전망으로), audio_usage 동의와 녹음의 연결. 확정 전까지 Worker는 텍스트 3축으로만 동작하고 delivery_score는 null이다.
...
- **회원 탈퇴 파기**: 탈퇴 후 3일 경과 시 해당 회원의 리포트 데이터 전체(REPORTS·REPORT_SCORES·REPORT_FEEDBACKS·REPORT_GENERATION_JOBS)를 완전 삭제한다(이력서와 동일 정책, DELETION_LOG 파기 대상에 포함).
...
- 정렬(`sort`): `createdAt`(기본, 내림차순) 또는 `overallScore`. `order`: `desc`(기본)/`asc`.
...
- 응답 구성: 리포트 메타(스냅샷·완료 시각) + 총평(`summary`) + 축별 점수·overall(REPORT_SCORES) + 질문 수 + 약점 태그 요약 + **개선 과제**(`improvementTasks` — 답변별 REPORT_FEEDBACKS.improvement_tasks를 질문 순서대로 모아 반환, 별도 저장 없음) + **AI 분석 한계 안내 문구**.
...
- **PENDING은 push하지 않는다** — 상태 이벤트는 Worker가 발행하며 PENDING은 Spring이 만드는 초기 상태다. 사용자는 REST 동기화(목록·상태 조회)로 PENDING을 인지하고, SSE 이벤트는 PROCESSING부터 흐른다. 재생성으로 PENDING에 복귀하는 경우도 재생성 API 응답이 그 사실을 전달하므로 push가 필요 없다.
