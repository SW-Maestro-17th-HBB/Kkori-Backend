...
- **중복 업로드 처리 (2층 구조)**:
  ...
  - **물리 삭제 배치와의 직렬화 (2026-09-11 HBB1-13)** — soft delete된 옛 이력서의 물리 삭제 배치(deletion.md 기능 5)가 같은 키의 객체를 지우는 것과 재업로드가 겹칠 수 있으므로, DB 저장 트랜잭션은 user 행 잠금을 잡은 뒤 **객체 존재를 재확인해 없으면 다시 저장**하고 행을 만든다. 배치는 같은 잠금 안에서 활성 참조를 확인한 뒤 객체를 지우므로, 어느 순서든 활성 이력서의 객체는 남는다.
