...
수정·재분석의 위치는 **면접 준비 전의 파싱 보정 단계**다 — "AI가 내 이력서를 잘못 읽은 부분"을 고치는 수단이지, 이력서 내용을 발전시키는 수단이 아니다. 이력서의 진실 원천은 PDF 원본이므로 내용 개선은 새 PDF 업로드(새 이력서 레코드)로 하며, 이미 면접·리포트에 사용된 이력서 레코드의 파싱 결과를 이후에 수정하는 것은 지원 시나리오가 아니다(2026-07-22 확정 — 막지는 않되, 리포트는 평가 당시 스냅샷을 보존하므로 영향받지 않는다).
...
사용자가 이력서를 삭제하면(`DELETE /api/v1/resumes/{resumeId}`) 이력서 원본 파일(S3), 구조화 데이터, 청크, 임베딩이 삭제 대상으로 표시된다. MVP에서는 **soft delete만** 수행한다(목록·조회에서 제외). 개별 삭제된 이력서의 물리 삭제 배치는 후속 스토리에서 정한다(HBB1-13에서 분리 — deletion.md 기능 5 절). 탈퇴 시에는 별도 경로인 deletion.md 기능 3(탈퇴 파기)이 soft delete 이력서를 포함해 전부 파기한다.
...
- (후속 스토리 — 물리 삭제 배치) 물리 삭제 시 S3 원본, structuredData, resume_chunks(임베딩 포함)가 모두 제거되는지 확인
- (후속 스토리 — 물리 삭제 배치) 같은 objectKey를 참조하는 다른 활성 이력서가 있으면 S3 객체는 삭제하지 않는지 확인 (해시 기반 키는 여러 레코드가 한 객체를 공유하므로 참조 확인 필수)
...
- soft delete 후 물리 삭제 배치의 주기: **미정** (HBB1-13에서 후속 스토리로 분리 — deletion.md 기능 5 절 참조)
