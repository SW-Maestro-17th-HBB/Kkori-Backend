...
- 세션이 생성되면 그 이력서는 "진행 중 면접에서 사용 중"이 된다 — 판정 기준은 **해당 이력서를 참조하는 non-terminal 세션의 존재**다. `resume.md`(§4·§5)가 이미 요구하는 사용 중 이력서의 수정·재분석·삭제 차단(`409 RESUME_IN_USE`)은 이 판정 기준이 있어야 성립하므로, **이력서 측 검사 활성화를 본 스토리 범위에 포함**한다. 적용 대상은 현존 경로인 **수정·재분석**이다 — 삭제 API(`resume.md` §5)는 아직 구현되지 않았으므로, 삭제 측 검사는 삭제 기능을 구현하는 스토리에서 동일 판정 쿼리를 재사용한다.
...
- non-terminal 세션이 참조하는 이력서의 수정·재분석 요청이 `409 RESUME_IN_USE`로 거부되는지, 그 세션이 terminal이 되면 다시 허용되는지 확인 (삭제 측 검사는 삭제 API 구현 스토리에서)
...
- **처리 순서**: 기존 `PENDING` 세션 `ABORTED` 처리(기능 1, DB만) → 세션 레코드 저장(id 확보) → 신규 룸 생성(LiveKit API) → 토큰 발급(로컬 서명) → 커밋 → **커밋 성공 후** 교체된 기존 세션의 룸 삭제 → 응답.
- **실패 시 보상**: 룸 생성·토큰 발급이 실패하면 트랜잭션 전체가 롤백되어 세션 레코드도, 기존 세션의 `ABORTED` 전이도 남지 않는다(시끄러운 실패 — 유저 재시도로 복구, `resume.md`의 트랜잭션 내 발행과 동일 철학). 롤백이 확정된 실패에는 **신규 룸 삭제를 best-effort로 보상 시도**한다 — 룸 생성 호출 실패도 포함한다(타임아웃은 "룸이 안 만들어졌다"가 아니라 "응답을 못 받았다"이므로 룸이 실제로 존재할 수 있다; 미생성 룸에 대한 삭제 시도는 무해). 단 **커밋 여부를 알 수 없는 실패**에는 아무 룸도 삭제하지 않고 세션 id·룸 이름을 로그로 남긴다 — 커밋됐을 수 있는 세션의 룸을 파괴하지 않기 위함이며, 잔여 룸은 empty timeout이 수용한다.
- **기존 세션의 룸은 트랜잭션 안에서 삭제하지 않는다** — 트랜잭션 중 삭제하면 롤백 시 기존 세션은 non-terminal(`PENDING`)로 복귀하는데 룸만 사라져 세션↔룸 정합이 깨진다. `ABORTED`가 확정되는 커밋 성공 후에만 삭제한다.
...
- **LiveKit API 타임아웃**: 룸 생성·삭제 호출에는 짧은 연결·응답 타임아웃을 명시적으로 설정하고 재시도하지 않는다. 타임아웃·실패의 결과는 호출 맥락별로 다르다(위 실패 보상 규칙과 동일) — **신규 룸 생성**은 `SESSION_ROOM_CREATE_FAILED`(S002) + 트랜잭션 롤백, **커밋 후 기존 룸 삭제**는 로그만 남기고 정상 응답 유지, **롤백 경로의 보상 삭제**는 로그만 남기고 원래 오류(S001/S002) 유지. 룸 생성은 user 행 잠금을 보유한 채 일어나는 외부 왕복이므로, 타임아웃이 잠금 보유 시간의 상한이 되어 LiveKit 지연·장애가 동일 유저의 다른 경로(탈퇴·정보 수정 등)를 길게 막지 못하게 한다. 값은 설정 프로퍼티 `livekit.api-timeout`으로 관리한다(local 기본 3s, dev/prod는 기본값 없는 환경변수 — 기존 `livekit.*` 방침과 동일).
...
- 룸 생성 실패 시(어댑터 예외·타임아웃) `500 SESSION_ROOM_CREATE_FAILED`가 반환되고, 세션 레코드가 남지 않으며(롤백) 기존 `PENDING` 세션도 `ABORTED`로 바뀌지 않고, 생성을 시도한 신규 룸에 보상 삭제가 시도되는지 확인 (타임아웃은 룸이 만들어졌을 수 있음)
- 토큰 발급 실패 시 `500 SESSION_TOKEN_ISSUE_FAILED`가 반환되고 세션 레코드가 남지 않으며, 이미 생성된 신규 룸에 보상 삭제가 시도되는지 확인
- 트랜잭션이 롤백되는 경우 기존 세션의 룸 삭제가 호출되지 않는지 확인 (기존 룸 삭제는 커밋 성공 후에만)
...
- 이력서 사용 중 차단은 이력서 도메인에 `RESUME_IN_USE`(409, R012)를 **신설**해 사용한다 — `resume.md`가 정의만 해둔 코드를 본 스토리가 도입·이행한다.
