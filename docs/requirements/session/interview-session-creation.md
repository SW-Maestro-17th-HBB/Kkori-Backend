...
- **처리 순서**: [트랜잭션] 기존 `PENDING` 세션 `ABORTED` 처리(기능 1) → 세션 레코드 저장(id 확보) → **커밋(user 잠금·DB 커넥션 해제)** → [트랜잭션 밖] 신규 룸 생성(LiveKit API) → 토큰 발급(로컬 서명) → 교체된 기존 세션의 룸 삭제(best-effort) → 응답. **LiveKit 왕복은 트랜잭션·잠금 밖에서 수행한다** — 직렬화가 필요한 구간은 순수 DB 작업뿐이므로, 외부 응답을 기다리는 동안 DB 커넥션과 user 행 잠금을 들지 않는다(LiveKit 지연이 커넥션 풀·동일 유저의 다른 경로에 전파되지 않음).
- **실패 시 처리**: 트랜잭션 안(검증·교체·저장)의 실패는 전체 롤백되어 아무 흔적도 남지 않는다(시끄러운 실패 — 유저 재시도로 복구). **커밋 이후의 실패**(룸 생성 `S002`·토큰 발급 `S001`)는 500으로 응답하되, **커밋된 신규 `PENDING` 세션이 룸 미보장 상태로 남고 교체된 기존 세션의 `ABORTED`도 확정된 채 유지된다** — 이 잔여물은 별도 정리 장치 없이 다음 생성 요청의 PENDING 자동 교체가 수렴시킨다(재시도가 곧 복구 경로). 실패한 신규 룸은 best-effort로 보상 삭제한다 — 룸 생성 타임아웃은 "룸이 안 만들어졌다"가 아니라 "응답을 못 받았다"일 수 있고(미생성 룸 삭제 시도는 무해), 토큰 실패 시엔 룸이 실제로 존재하기 때문이다.
...
- 룸 생성 실패 시(어댑터 예외·타임아웃) `500 SESSION_ROOM_CREATE_FAILED`가 반환되고, **커밋된 신규 `PENDING`이 남으며 교체된 기존 세션은 `ABORTED`로 확정**되는지, 생성을 시도한 신규 룸에 보상 삭제가 시도되는지 확인 (타임아웃은 룸이 만들어졌을 수 있음)
- 토큰 발급 실패 시 `500 SESSION_TOKEN_ISSUE_FAILED`가 반환되고 **커밋된 신규 `PENDING`이 남으며**, 이미 생성된 신규 룸에 보상 삭제가 시도되는지 확인
- 룸·토큰 실패 후 같은 유저의 재시도가 잔존 `PENDING`을 자동 교체하고 정상 생성되는지 확인 (실패 잔여물의 수렴 경로)
- 트랜잭션 안의 실패(검증 거부 등)로 롤백되는 경우 어떤 룸 작업도 호출되지 않는지 확인 (룸 작업은 커밋 후에만)
