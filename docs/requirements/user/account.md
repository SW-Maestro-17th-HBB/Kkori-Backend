...
| 4 | 계정 복구·유예 만료 처리 | 카카오 로그인 시 탈퇴 계정에 해당 탈퇴 건에 바인딩된 signup token을 발급하고, 재동의 제출(`/auth/signup`)의 잠금 하 재판정에서 유예 내면 복구, 유예 초과면 식별정보 파기(provider_id 마스킹) 후 신규 생성해야 한다. |
...
- 검증: `Authorization: KakaoAK {대표 어드민 키}` 헤더가 서버 설정의 어드민 키와 일치하고 `app_id`가 우리 앱 ID와 일치할 때만 탈퇴 동기화를 수행한다. 불일치 시 **상태 변경 없이 보안 로그(WARN 이상, 수신 파라미터 포함 — 단 `Authorization` 헤더·어드민 키 원문은 제외하고, `user_id`는 전용 키 기반 HMAC 가명값으로 기록)만 남기고 동일하게 `200 OK`를 반환한다** — 카카오는 200 외 응답을 실패로 간주해 재전송하며 연속 실패 시 웹훅을 비활성화하므로, 401 등 에러 응답은 사용하지 않는다(카카오 웹훅 명세). 검증 실패 탐지는 내부 모니터링(로그 기반)이 담당한다.
...
- 연결 해제 웹훅에는 **재전송이 지원되지 않는다**(카카오의 단계별 재전송은 계정 상태 변경 웹훅 전용). 따라서 처리 실패를 카카오 재전송으로 복구할 수 없으며, 탈퇴 동기화 처리 중 오류는 에러 로그(ERROR 레벨, `user_id`의 HMAC 가명값 포함)로 남겨 내부 모니터링으로 탐지한다. **모든 웹훅 로그에서 `user_id` 원문은 기록하지 않는다** — 카카오 회원번호는 안정적 외부 식별자로, 원문이 로그 수집 시스템에 장기 보관되는 것을 방지한다(가명값은 동일 입력에 동일 값이라 상관관계 추적은 유지됨).
...
- 어드민 키 불일치 또는 `app_id` 불일치 시에도 200이 반환되되, 어떤 유저도 변경되지 않고 보안 로그(WARN 이상)가 남는지 확인 — 로그에 `Authorization` 헤더·어드민 키·`user_id` 원문이 나타나지 않는지 함께 확인
- 웹훅 로그의 `user_id`가 모든 경로(성공·미존재·이미 탈퇴·검증 실패·처리 오류)에서 HMAC 가명값으로 기록되고 원문이 나타나지 않는지 확인
...
- 환경 변수: 로그 가명화 전용 HMAC 키(`LOG_HMAC_KEY`) — 인증·서명 키와 공유 금지(용도 분리), 동일 fail-fast 패턴
...
- **[MVP 잔여 위험]** 연결 해제 웹훅은 재전송이 없어 단 한 번의 수신·처리 실패로도 유실될 수 있다. 유실 시 유저가 서비스 외부에서 표시한 연결 해제 의사가 반영되지 않아, **계정과 기존 AT·RT가 활성 상태로 유지되고 개인정보 파기가 시작되지 않는다**. MVP의 대응은 아래로 한정하고 나머지는 명시적 잔여 위험으로 수용한다.
  - **수신 후 처리 실패**: ERROR 로그(`user_id` 가명값 포함)로 탐지해 수동 조치한다(기타 요구사항의 운영 절차).
