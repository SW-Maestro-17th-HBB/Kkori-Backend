...
- 검증: `Authorization: KakaoAK {대표 어드민 키}` 헤더가 서버 설정의 어드민 키와 일치하고 `app_id`가 우리 앱 ID와 일치할 때만 탈퇴 동기화를 수행한다. 불일치 시 **상태 변경 없이 보안 로그(WARN 이상, 수신 파라미터 포함 — 단 `Authorization` 헤더·어드민 키 원문은 제외)만 남기고 동일하게 `200 OK`를 반환한다** — 카카오는 200 외 응답을 실패로 간주해 재전송하며 연속 실패 시 웹훅을 비활성화하므로, 401 등 에러 응답은 사용하지 않는다(카카오 웹훅 명세). 검증 실패 탐지는 내부 모니터링(로그 기반)이 담당한다.
...
- 어드민 키 불일치 또는 `app_id` 불일치 시에도 200이 반환되되, 어떤 유저도 변경되지 않고 보안 로그(WARN 이상)가 남는지 확인 — 로그에 `Authorization` 헤더·어드민 키 원문이 나타나지 않는지 함께 확인
