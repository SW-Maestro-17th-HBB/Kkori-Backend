...
- 행 없이 S3에만 남은 고아 객체(`resumes/{userId}/` 하위)도 삭제되는지 확인
...
- 중간 단계(예: 리포트)에서 실패한 건을 재시도하면 이미 끝난 단계는 건너뛰고 나머지를 완료해 `PURGED`가 되는지 확인
...
- 지연 시간의 근거: 업로드 경로(`ResumeUploadService`)는 "S3 객체 존재 확인 → DB 저장" 순서라, 같은 사용자·같은 파일의 재업로드와 배치의 S3 삭제가 그 사이에 겹치면 새 행이 사라진 객체를 가리킬 수 있다. 업로드는 수 초 내 끝나므로 10분 지연이 이 창을 흡수한다.
...
**처리**(건별, user 행 잠금 무필터 선행 — 이력서 수정·삭제·세션 생성 경로와 직렬화):
...
- 예외 메시지(`lastError`·로그)는 S3·카카오 응답 본문을 그대로 싣지 않고 예외 클래스와 요약으로 제한한다(응답에 회원번호가 포함될 수 있음 — unlink 성공 응답의 `id`).
