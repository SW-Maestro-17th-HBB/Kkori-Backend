...
- **함께 다루는 인접 항목**: (1) 탈퇴 시점의 진행 중 세션 즉시 종료 — 세션 PRD 3종이 "E1 연계 후속 스토리"로 위임, (2) 개별 이력서 삭제의 물리 삭제 배치 — 처음 포함했다가 **후속 스토리로 분리**(기능 5 절), (3) Refresh Token 청소 배치 — 설계 초안 ADR-013 미구현, (4) 동의 이력 보존 정책 확정 — consent.md가 본 스토리에 위임.
...
| 5 | 개별 이력서 물리 삭제 배치 | **후속 스토리로 분리** — 본 스토리 범위 밖(아래 기능 5 절 참조). |
...
- Worker·에이전트의 뒤늦은 쓰기(파기 후 청크 INSERT·리포트 피드백 INSERT 등)는 유예 3일이 파이프라인 소요(수 분)를 압도하므로 발생하지 않는다고 본다. 발생하더라도 고아 청크·리포트 산출물의 고아 행은 운영 점검으로 정리한다 — 수용 잔여 위험. 근본 대책(Worker가 쓰기 직전 행 존재를 확인)은 Kkori-AI 후속 과제로 권고한다.
...
- **호출 직전 소유권 재확인**: unlink는 되돌릴 수 없는 외부 호출이라, 호출 직전에 `deletion_log`가 여전히 이 실행자의 선점(`status = PURGING AND updated_at = 선점 시각`)인지 읽어 확인하고 아니면 호출하지 않는다(소유권 상실 — 결과 폐기, 기능 2). 완전 차단은 불가하다 — 카카오에 조건부 unlink가 없어 확인~호출 사이의 창(밀리초)은 남는다. 그 창에서 stale 실행자가 재가입한 새 연결을 끊으려면 실행자가 재선점 임계(`purging-timeout`) 이상 멈춘 뒤 같은 사용자가 재가입까지 마쳐야 하고, 영향은 새 연결의 재동의뿐이다(데이터 파기는 DB 펜싱이 보호) — 수용 잔여 위험.
...
- 재선점된 건(선점 시각 불일치)에서는 unlink가 호출되지 않고 스냅샷이 유지되는지 확인
...
## 개별 이력서 물리 삭제 배치 — 후속 스토리로 분리

### 설명

`DELETE /api/v1/resumes/{resumeId}`로 soft delete된 이력서를 배치가 물리 삭제하는 기능은 본 스토리에 포함해 구현했다가 **후속 스토리로 분리했다**(2026-09-21). 개별 삭제 배치는 활성 유저의 이력서를 지우므로 같은 유저의 재업로드와 **S3 키(해시 기반 — resume.md §1)를 공유**하는 경합이 생기고, 이를 막는 직렬화가 업로드·배치 양쪽의 DB 트랜잭션 안에 S3 호출을 두게 된다(리뷰 지적 — 트랜잭션 안 외부 호출 지양). 유력한 해결은 키 설계 변경(업로드마다 고유 키)인데 HBB1-14의 결정을 바꾸는 일이라 별도 스토리에서 PRD부터 확정한다.

- 후속 스토리가 정할 것: S3 키 규칙(공유 해시 키 유지 vs 업로드별 고유 키), 배치 주기·지연, 분석 진행 중 이력서의 처리(terminal 대기·상한), 상한 삭제 뒤 Worker가 남기는 고아 청크 정리.
- 그때까지 resume.md §5의 "물리 삭제 배치의 주기: 미정"은 유지되며, 개별 삭제된 이력서의 데이터는 그 유저의 탈퇴 파기(기능 3 — soft delete 이력서 포함)까지 남는다(develop과 같은 상태).
- 번호 5는 타 문서의 기능 6·7 참조를 유지하기 위해 비워 둔다.
...
- 두 조건은 단순 벌크 DELETE로 수행하며 잠금이 필요 없다. 두 DELETE는 **각각의 트랜잭션**으로 실행한다(탈퇴의 RT 일괄 폐기 UPDATE와 잠금 순서가 엇갈려 교착할 가능성을 문장 단위로 좁힌다 — 교착의 반대편이 재전송 없는 웹훅일 수 있다). 조건이 시각 기준이라 다중 인스턴스 동시 실행도 중복 삭제 시도가 0행으로 수렴한다. 재발급 경로의 `FOR UPDATE` 잠금과의 경합은 DELETE가 잠금 해제까지 대기하는 것으로 무해하다(폐기 직후 토큰은 보존 기간 안이라 삭제 대상이 아니다).
...
- 두 배치(파기·RT 청소)는 모두 앱 내 `@Scheduled(fixedDelayString = …)`로 실행한다(`SchedulingConfig`의 `@EnableScheduling` 기존 활성). 회차 시각은 주입된 `Clock`에서 취득한다.
- **스케줄러 스레드 풀**: Spring Boot의 기본 스케줄러 풀 크기는 1이라 세션 스위퍼(10초 주기)·SSE keepalive(20초 주기)·본 스토리의 배치 2종이 한 스레드를 순서대로 나눠 쓴다. 파기 회차가 S3·카카오 왕복으로 수십 초를 점유하면 그동안 스위퍼가 밀려 세션 수렴이 지연된다. `spring.task.scheduling.pool.size`를 주기 작업 수 이상으로 올리거나 파기 배치에 전용 실행기를 지정한다 — 환경별 운영값이 아니므로 공통 `application.yaml`에 둔다. 검증: 파기 회차가 진행 중인 동안 스위퍼 회차가 지연되지 않는지 확인.
- **종료 시 대기**: 앱 종료(배포) 시 진행 중인 회차를 `spring.task.scheduling.shutdown.await-termination`(대기 30초)으로 기다린다 — 회차 중간에 스레드가 중단되면 선점한 건이 `PURGING`으로 남아 `purging-timeout`(30분)까지 어느 인스턴스도 회수하지 못한다. 각 트랜잭션은 짧아 정합은 깨지지 않으며 대기는 지연만 줄인다. 공통 `application.yaml`.
...
- 테스트에서는 배치 빈을 등록하지 않고(`app.batch.enabled=false` — 테스트 클래스패스의 `EnvironmentPostProcessor`가 빈 조건 평가 **전에** 주입하는 테스트 전용 스위치, 기본 true) 스케줄 메서드를 직접 호출해 회차를 검증한다(스위퍼 테스트 관례). 스케줄러 빈 부재는 테스트가 단언한다. 통합 테스트가 유예 초과 시나리오용으로 시딩하는 과거 시각의 탈퇴 건을 백그라운드 회차가 실제로 파기하지 않게 하기 위함이다.
...
- 배포 매니페스트(환경변수 목록)에 공통: 배치 실행·설정의 신규 환경변수 5종을 추가한다.
