...
- Worker·에이전트의 뒤늦은 쓰기(파기 후 청크 INSERT·리포트 피드백 INSERT 등)는 유예 3일이 파이프라인 소요(수 분)를 압도하므로 발생하지 않는다고 본다. 발생하더라도 고아 청크는 개별 이력서 물리 삭제 배치의 고아 청크 정리(기능 5)가 흡수하고, 리포트 산출물의 고아 행은 운영 점검으로 정리한다 — 수용 잔여 위험은 후자로 한정.
...
- **업로드 경로와의 직렬화(공유 키 보호)**: 지연만으로는 배치 실행 중의 재업로드를 보호하지 못한다 — 배치의 "활성 참조 확인 → S3 삭제" 사이에 같은 사용자·같은 파일의 재업로드가 "객체 존재 확인(있음 → 저장 생략) → 행 INSERT"를 끼워 넣으면 새 행이 사라진 객체를 가리킨다. 그래서 양쪽을 **같은 사용자·같은 해시의 soft delete 행 잠금(FOR UPDATE)** 으로 직렬화한다: 배치는 후보 행을 잠근 채 참조 확인 → S3 삭제 → 행 삭제를 한 트랜잭션으로 수행하고(이 단계에 한해 S3 삭제가 잠금 안에 있다), 업로드는 잠금 밖의 저장 뒤 **같은 해시의 soft delete 행을 잠그고 객체 존재를 재확인해 없으면 다시 저장**한 뒤 행을 INSERT한다(resume.md §1 — 그런 행이 없는 첫 업로드는 잠글 것이 없다). 배치가 먼저면 업로드가 재저장하고, 업로드가 먼저면 배치가 활성 참조를 보고 객체를 남긴다. **users 행 잠금은 쓰지 않는다** — S3 왕복 동안 users 행을 쥐면 같은 유저의 탈퇴 웹훅(2초 트랜잭션, 재전송 없음)이 잠금 대기로 실패해 탈퇴가 누락될 수 있다. soft delete 행은 웹훅·계정 경로가 건드리지 않으며, 탈퇴 파기(기능 3)의 행 삭제는 이 잠금을 기다렸다가 대상 없음으로 끝난다.
- 지연 시간의 근거: 업로드가 잠금 밖에서 수행하는 첫 저장과 잠금 획득 사이의 창, 그리고 삭제 직후의 파이프라인 잔여 처리를 흡수한다. 위 직렬화가 정합을 보장하므로 지연은 재저장(낭비)을 줄이는 완충일 뿐이다.
- 분석 terminal 조건의 근거: 삭제 API는 분석 진행 중 이력서도 삭제를 허용한다. 진행 중에 청크·행을 지우면 Worker가 그 뒤에 청크(이력서 본문 — 개인정보)를 INSERT해 고아로 남는다. 상한(6배)은 Worker가 상태를 영영 전이하지 않는 병리(Worker 중단)에서도 삭제가 수렴하게 하는 백스톱이다.
- **고아 청크 정리**: 상한 삭제 뒤 Worker가 뒤늦게 남긴 청크는 `resumes` 행이 없어 어떤 배치도 다시 찾지 못하므로(영구 잔존 — 개인정보라 수용 불가), 매 회차 끝에 **`resumes`에 없는 `resume_id`를 가진 청크 중 생성 후 지연 시간이 지난 것**을 삭제한다. 생성 직후 청크를 제외하는 이유: 업로드는 분석 요청을 행 커밋과 같은 트랜잭션에서 발행하므로 Worker가 행 커밋보다 먼저 청크를 쓰는 아주 짧은 창이 이론상 있고, 그 청크는 고아가 아니다. 근본 대책(Worker가 쓰기 직전 행 존재를 확인·삭제된 이력서의 작업 취소)은 Kkori-AI 후속 과제로 권고한다 — 두 대책은 상충하지 않는다.

**처리**(건별, 후보 행 잠금(FOR UPDATE) 선행 — 업로드의 잠금 내 재확인과 직렬화. 아래 1·2를 잠금 트랜잭션 하나에서 수행하며, S3 삭제 실패는 트랜잭션째 되돌아가 행이 남는다. 잠금 시점에 행이 이미 없으면 — 탈퇴 파기·타 인스턴스 — 건너뛴다):
...
- 배치의 S3 삭제가 오래 걸리는 동안에도 같은 유저의 탈퇴 웹훅 트랜잭션(2초 타임아웃)이 대기 없이 완료되는지 확인 (users 행 잠금 미사용)
- `resumes` 행이 없는 고아 청크 중 생성 후 지연 시간이 지난 것은 회차 끝에 삭제되고, 행이 있는 이력서의 청크와 생성 직후의 고아 청크는 유지되는지 확인
...
- 회차 처리량 상한 없음(MVP 규모). 건별 트랜잭션은 후보 soft delete 행 잠금(FOR UPDATE) 하나로 참조 확인 → S3 삭제(단일 호출) → 행 삭제를 수행한다 — S3 왕복 중 users 행은 잠그지 않는다. "S3 왕복은 잠금 밖"은 탈퇴 파기(기능 3)의 계약이며 이 배치에는 적용되지 않는다.
