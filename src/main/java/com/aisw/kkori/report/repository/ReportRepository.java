...
public interface ReportRepository extends JpaRepository<Report, Long> {
    ...
    /**
     * 리포트 행을 잠그고 읽는다(SELECT ... FOR UPDATE) — 상태 확인 후 행동하는 재생성 트랜잭션끼리
     * 직렬화해 낡은 판정으로 통과하는 경합(연속 클릭 등)을 막는다. 락은 이름이 아니라 {@code @Lock}이
     * 건다 — ForUpdate는 서술일 뿐이므로 애너테이션을 지우면 이름만 남고 락은 조용히 사라진다.
     */
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    Optional<Report> findForUpdateById(Long id);
